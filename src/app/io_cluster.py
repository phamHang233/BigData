...
def save_dataframes_to_elasticsearch(dataframes, indices, es_write_config):
    ...
    for dataframe, index in zip(dataframes, indices):
        ...
        rdd_ = dataframe.rdd
        rdd_.map(lambda row: (None, json.dumps(row.asDict()))).saveAsNewAPIHadoopFile(path='-',
                                                                                      outputFormatClass="org.elasticsearch.hadoop.mr.EsOutputFormat",
                                                                                      keyClass="org.apache.hadoop.io.NullWritable",
                                                                                      valueClass="org.elasticsearch.hadoop.mr.LinkedMapWritable",
                                                                                      conf=es_write_config)
def save_df_to_elastic(dataframes, indices, config):
    for dataframe, index in zip(dataframes, indices):
        # Lưu DataFrame vào Elasticsearch
        print(config.elasticsearch_conf['es.nodes'],config.elasticsearch_conf["es.port"])
        dataframe.write.format("org.elasticsearch.spark.sql").option("es.nodes", config.elasticsearch_conf['es.nodes']).option("es.port", config.elasticsearch_conf["es.port"]).option("es.resource", index).mode("overwrite").save()
        print("save to elastic sucessful!!!!")
